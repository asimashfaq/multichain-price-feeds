import {
  ChainlinkFeedsContract_AnswerUpdated_loader,
  ChainlinkFeedsContract_AnswerUpdated_handler,
} from "../generated/src/Handlers.gen";

import { priceEntity, feedEntity } from "../generated/src/Types.gen";

ChainlinkFeedsContract_AnswerUpdated_loader(({ event, context }) => {
  // try load the feed from the event into memory
  context.Feed.load(event.srcAddress.toString());
});

ChainlinkFeedsContract_AnswerUpdated_handler(({ event, context }) => {
